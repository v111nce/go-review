...
| --- | --- | --- | --- | --- |
| STORY-001 `tool-adapter-platform-core` | [工具接入平台](../product/go-code-quality-governance.md#工具接入平台) | CLI/config/adapter core | verified | `go test ./...`; `internal/config`, `internal/engine`, `internal/adapter`; fixture config runs `cmd`, `go.lint`, and `go.semantic` adapters. |
...
| STORY-005 `custom-rule-adapters-sdk` | [自定义规约 Adapter](../product/go-code-quality-governance.md#自定义规约-adapter) | `go/analysis` analyzer runtime for semantic adapter | verified | `go.semantic` detects direct `os.Getenv` and configurable `max-params`; `semantic-violating-project` fails with rule `semantic.no-direct-os-getenv`, file/line/column, reason, suggestion, and `review` safety. |
...
| STORY-007 `policy-and-autofix-safety-levels` | [策略与安全自动修复](../product/go-code-quality-governance.md#策略与安全自动修复) | Canonical safety + CLI display | verified | `ParseFixSafety` aliases and `internal/fix.Policy` precedence tests pass; CLI prints `fix_available` and `fix_safety`; `fix` applies only allowed `safe` `go.lint` format step. |
| STORY-008 `policy-and-autofix-transaction` | [策略与安全自动修复](../product/go-code-quality-governance.md#策略与安全自动修复) | Safe format application + downstream failure reporting | verified | `internal/fix` covers overlap/rollback; engine test keeps formatted file when dependent `go test` fails; autofix fixture passes check→fix→test. |
| STORY-009 `regression-gates-local-fast-check` | [回归门禁](../product/go-code-quality-governance.md#回归门禁) | Fast check/fix profile | verified | `go run ./cmd/go-review check --profile fast --workdir <abs>/compliant-project` passed; `fix --profile fast` on `autofix-project` applies safe `golangci-lint fmt` formatting and validates tests. |
| STORY-010 `regression-gates-pr-quality-check` | [回归门禁](../product/go-code-quality-governance.md#回归门禁) | CI/main profile | verified | `go run ./cmd/go-review check --profile ci --workdir <abs>/violating-project` exited non-zero on `go.lint` format and emitted artifact paths. |
...
| Safe autofix smoke | verified | `fix --profile fast` on `autofix-project` applied `golangci-lint fmt`, reran tests, and passed. |
| Safe fix retention | verified | Engine unit test keeps formatting when dependent validation fails; `internal/fix` unit tests still cover overlap and validator rollback inside one edit transaction. |
| Docs traceability | verified | `README.md`, `docs/backend/rule-engine-and-autofix.md`, fixture README, and this file map implementation evidence back to product/backend/quality docs. |
...
- The semantic adapter runs `go/analysis` analyzers, not a dynamic third-party plugin loader or arbitrary YAML DSL; custom semantic rules need implemented analyzer kinds and run through `go.semantic`.
