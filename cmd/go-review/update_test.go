...
func TestRunUpdateAppliesBinaryAndMissingConfig(t *testing.T) {
	...
	for _, want := range []string{"发现新版本", "UPDATED go-review v0.1.0 -> v0.2.0", "adapters[id=llm.claude]", "steps[id=llm-claude]", "profiles[review].steps += llm-claude", "rules/llm-default.json", "rules/llm-custom.json"} {
	...
	for _, path := range []string{filepath.Join(dir, ".go-review", "rules", "llm-default.json"), filepath.Join(dir, ".go-review", "rules", "llm-custom.json")} {
